"""
users.py — User Management router (security-hardened)

Security fixes applied:
- FIX-3: All endpoints now require admin JWT token via require_admin dependency
- FIX-5: Password hashing uses bcrypt (imported from security module)
"""
...
def list_users():
    ...
    FIX-3: Requires valid admin JWT.
    """
    conn = get_db_connection()
    with conn.cursor() as cursor:
        cursor.execute(
            "SELECT id, username, full_name, email, role, account_type, azure_oid, created_at FROM users ORDER BY created_at DESC;"
        )
...
def create_user(payload: dict = Body(...)):
    ...
    FIX-3: Requires valid admin JWT.
    FIX-5: Passwords hashed with bcrypt.
    """
    ...
    if account_type == "microsoft":
        if not email or not full_name:
            raise HTTPException(
                status_code=400,
                detail="Email address and Full Name are required for Microsoft user",
            )
        if not username:
        ...
        if not username or not full_name or not email or not password:
            raise HTTPException(
                status_code=400,
                detail="All fields (Username, Full Name, Email, Password) are required for System user",
            )
        # FIX-5: bcrypt hash
